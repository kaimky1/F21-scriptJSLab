...
const ohMy = () => {
    ...
    .then((res) => {
        for(let i = 0; i < res.data.length; i++){
            ...
            // res.data[i] = document.createElement('p').textContent;
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/kai')
    .then(res => {
        console.log(res.data)
        res.data;
...
const getQueryRequest = () => {
    axios.get('http://localhost:3000/query-test?person=kai&person=jane')
    .then(res => {
        console.log(res.data)
    }).catch(error => {
        console.log(error)
...
queryBtn.addEventListener('click', getQueryRequest);
...
/*
    ...
    You can send more than 1 query and it seperates it by commas. 
*/
